...
/**
 * postPathBySlugCategory
 *
 * Articles all live at /blog/{slug}. The category argument is retained so
 * existing call sites keep working, but it no longer affects the path —
 * legacy /{category}/{slug} URLs are 301'd from next.config.js.
 *
 * @deprecated Use postPathBySlug instead.
 */

export function postPathBySlugCategory(slug) {
  return postPathBySlug(slug);
}
...
export function mapPostData(post = {}) {
  ...
  // Same flattening for tags, so consumers read post.tags[].slug directly
...
export async function getRelatedPosts(categories, postId, count = 3) {
  ...
  if (category?.databaseId) {
    ...
    const filtered = (Array.isArray(posts) ? posts : []).filter(
      ({ postId: id, slug }) => id !== postId && slug !== postId
    );

    related.posts = sortObjectsByDate(filtered).slice(0, count);
  }

  // A category with nothing but the current post in it is no use, so fall
  // through to the post's next category.
  if (related.posts.length === 0) {
    return getRelatedPosts(remainingCategories, postId, count);
  }
 ...
 * toMoreStoriesPosts
 *
 * Re-nests mapped post data back into the `{ node }` edge shape that
 * MoreStories / PostPreview consume.
 */

export function toMoreStoriesPosts(posts = []) {
  ...
  return posts.map((post) => ({
    node: {
      ...
      featuredImage: post.featuredImage ? { node: post.featuredImage } : null,
      author: post.author?.avatar?.url ? { node: post.author } : null,
    },
  }));
}
