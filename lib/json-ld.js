...
// The site has no per-author archive; /about is the canonical page describing
// Mark Laurie, so it serves as the author's identity reference.
const AUTHOR_PROFILE_URL = `${SITE_ORIGIN}/about`;
...
// WordPress excerpts arrive as HTML; schema.org description and meta tags
...
export function ArticleJsonLd({ post = {}, siteTitle = SITE_NAME }) {
  ...
  if (authorName) {
    // A bare name is not an identity reference. Point the author at the About
    // page so the Person is resolvable and can be reconciled across articles.
    jsonLd.author = {
      "@type": "Person",
